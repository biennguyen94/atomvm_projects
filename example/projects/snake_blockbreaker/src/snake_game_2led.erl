...
-export([start/1, joystick/3, blink_food/1, game_over_process/3, welcome_snake_game_process/2, variable_resistor/3]).
...
start(SPI) ->
    ...
    {ok, Pid} = gen_server:start(?MODULE, SPI, []),
    timer:sleep(500),
    JoystickPid = spawn(?MODULE, joystick, [Pid, ?GPIO_VRx, ?GPIO_VRy]),
    gen_server:cast(Pid, {update_joystick_pid, JoystickPid}),
...
handle_cast({update_joystick_pid, Pid}, State) ->
    {noreply, State#snake{joystick_pid=Pid}};

% Handle change direction request
% If Game over or New direction make Snake moving backward, ingore that request
handle_cast({change_direction, _X, _Y}, State) when State#snake.gameover == true ->
    {noreply, State};
...
handle_info(stop_peripherals, State) ->
    gpio:stop(),
    case is_pid(State#snake.joystick_pid) of
        true ->
            State#snake.joystick_pid ! stop;
        false ->
            ok
    end,
    {noreply, State};
...
handle_info(back_to_welcome, State) ->
    ...
    gen_server:cast(snake_blockbreaker, game_over),
    % {noreply, NewState}.
    {stop, normal, State}.
...
terminate(_Reason, _State) ->
    io:format("snake genserver terminated~n"),
    ok.
...
joystick(Pid, ADCX, ADCY) ->
    receive
        stop ->
            ok
    after ?DELAY_READ_ADC ->
        % io:format("Bien joystick: ~p~n", [Pid]),
        {ok, X} = read_adc(ADCX),
        {ok, Y} = read_adc(ADCY),
        if
            X < ?LOW_RANGE ->
