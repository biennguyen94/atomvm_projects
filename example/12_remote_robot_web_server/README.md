# `remote_robot_web_server` Application

Welcome to the `remote_robot_web_server` AtomVM application.

The `remote_robot_web_server` AtomVM application uses the `http_server` module and demonstrates retrieving and using variables from http input forms to control robot direction.
...
The `remote_robot_web_server` will accept input text and flash it out in morse code on the specified LED.  The default on-board LED for most ESP32 boards is GPIO 2.  You may also connect an LED to any GPIO pin and connect its anode to ground with a 100 Ohm or higher (220 Ohm recommended) resistor.
