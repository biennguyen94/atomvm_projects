...
-include("block_breaker_2led.hrl").

-export([start/1, joystick/3, variable_resistor/3, game_over_process/3,
...
handle_cast({update_joystick_pid, Pid}, State) ->
    {noreply, State#state{joystick_pid=Pid}};
...
handle_info(stop_peripherals, State) ->
    gpio:stop(),
    case is_pid(State#state.joystick_pid) of
        true ->
            State#state.joystick_pid ! stop;
        false ->
            ok
    end,
    {noreply, State};
