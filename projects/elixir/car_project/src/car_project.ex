defmodule CarProject do
  ...
  def handle_req("POST", [], conn) do
    ...
    duty_cycle = :proplists.get_value("duty", params)
  ...
  def handle_req("POST", ["forward"], conn) do
    ...
    IO.inspect({:forward, current_duty})
    car_forward()
  ...
  defp safe_list_to_integer(value) do
    case value do
      nil -> nil
      :undefined -> nil
      _ ->
  ...
  defp maybe_start_network(:esp32) do
    config = :config.get()
    sta = Map.get(config, :sta)

    case :network.wait_for_sta(sta, 30_000) do
      {:ok, {address, netmask, gateway}} ->
        IO.puts(
          "Acquired IP address: #{address_to_string(address)} Netmask: #{address_to_string(netmask)} Gateway: #{address_to_string(gateway)}"
        )
  ...
  defp init_peripheral do
    ...
    Enum.each(pins, fn gpio ->
      :gpio.set_pin_mode(gpio, :output)
    end)
  ...
  defp car_reset_pwm do
    ...
    if duty_channel_1 > duty_channel_2 do
      set_duty_channel_2(duty_channel_1)
    else
      set_duty_channel_1(duty_channel_2)
    end
  end
  ...
  defp set_duty_channel_1(duty_num) when is_integer(duty_num) do
    duty = round(duty_num * @cal_bit_div_100)
    :ok = :ledc.set_duty(@ledc_hs_mode, @ledc_hs_ch0_channel, duty)
    :ok = :ledc.update_duty(@ledc_hs_mode, @ledc_hs_ch0_channel)
  end

  defp set_duty_channel_1(_), do: :ok

  defp set_duty_channel_2(duty_num) when is_integer(duty_num) do
    duty = round(duty_num * @cal_bit_div_100)
    :ok = :ledc.set_duty(@ledc_hs_mode, @ledc_hs_ch1_channel, duty)
    :ok = :ledc.update_duty(@ledc_hs_mode, @ledc_hs_ch1_channel)
  end

  defp set_duty_channel_2(_), do: :ok

  defp get_duty_channel_1 do
    duty = :ledc.get_duty(@ledc_hs_mode, @ledc_hs_ch0_channel)
    round(duty * @cal_100_div_bit)
  end

  defp get_duty_channel_2 do
    duty = :ledc.get_duty(@ledc_hs_mode, @ledc_hs_ch1_channel)
    round(duty * @cal_100_div_bit)
  end

  defp address_to_string({a, b, c, d}), do: "#{a}.#{b}.#{c}.#{d}"
  defp address_to_string({address, port}), do: "#{address_to_string(address)}:#{port}"
