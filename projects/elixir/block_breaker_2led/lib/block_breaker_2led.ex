...
defmodule BlockBreaker2Led do
  ...
  defstruct [:spi, :crossbar, :ball, :direction, :point, :data1, :data2, :isgameover, :goverproc, :score]
  ...
  def handle_cast(:reset_game, state) do
    new_state = %{state | crossbar: @cross_bar, data1: @empty_matrix, data2: @empty_matrix, score: 0, goverproc: :undefined,
  ...
  def handle_cast(:move_ball, state) do
    new_state =
      if state.isgameover do
        ...
        {new_ball, new_direc, game_over, new_point, new_score} = move_ball(ball, state.direction, state.crossbar, state.point, state.score)
  ...
  def handle_info({:gpio_interrupt, @gpio_sw}, state) do
    ...
    case is_pid(state.goverproc) do
      true -> send(state.goverproc, :stop)
      false -> :ok
    end
  ...
  def handle_info(:back_to_welcome, state) do
    ...
    new_proc = spawn(__MODULE__, :welcome_block_breaker_game_process, [self(), 0])
    new_state = %{state | isgameover: true, goverproc: new_proc}
    {:noreply, new_state}
  end
  ...
  defp update_ball({id, {x, y}}, {pre_id, {pre_x, pre_y}}, data1, data2) do
    ...
    if id == pre_id do
      ...
      current_row = Map.get(data, x + 1)
      new_row = new_x ||| current_row
      result = Map.put(data, x + 1, new_row)
  ...
  defp read_adc(adc) do
    case :esp_adc.read(adc) do
      ...
      error -> IO.puts("Error taking reading: #{inspect(error)}")
    end
  end
  ...
  defp get_data(result, 9, _times, _command), do: result
  defp get_data(result, number, times, :welcome) do
    row = Map.get(@breaker_game, number + times)
    new_result = Map.put(result, number, row)
    get_data(new_result, number + 1, times, :welcome)
  end

  defp get_data(result, number, times, :lose) do
    row = Map.get(@game_over, number + times)
    new_result = Map.put(result, number, row)
    get_data(new_result, number + 1, times, :lose)
  end

  defp get_data(result, number, times, :win) do
    row = Map.get(@game_win, number + times)
    new_result = Map.put(result, number, row)
    get_data(new_result, number + 1, times, :win)
  end
  ...
  def variable_resistor(parent, adc, previous_speed) do
    ...
    map_speed = map(speed, 0, @bit_resolution, @max_speed, @min_speed)
  ...
  defp is_not_in_range(pre_val, new_val) do
    ...
    case (new_val >= low) and (new_val <= high) do
      true -> false
      false -> true
    end
  end
