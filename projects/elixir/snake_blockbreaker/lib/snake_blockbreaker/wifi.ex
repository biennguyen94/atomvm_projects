...
defmodule SnakeBlockbreaker.WiFi do
  @moduledoc """
  ...
  @dhcp_hostname "snake_blockbreaker"
  ...
  defp fetch_wifi_credentials do
    wifi_ssid = SnakeBlockbreaker.NVS.get_binary(:wifi_ssid)

    if is_nil(wifi_ssid) do
      IO.puts("wifi: missing SSID in NVS (key: wifi_ssid). Provision first.")
      {:error, {:missing_nvs_key, :wifi_ssid}}
    else
      wifi_passphrase = SnakeBlockbreaker.NVS.get_binary(:wifi_passphrase)
      {:ok, {wifi_ssid, wifi_passphrase}}
    end
  end
  ...
  def handle_sta_got_ip(ip_info) do
    ...
    SnakeBlockbreaker.DistErl.maybe_start(ip_info)
  end
